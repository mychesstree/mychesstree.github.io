...
import DailyGamePanel from '../components/DailyGamePanel';
...
export default function Dashboard() {
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showSuccess(`Successfully imported ${importedCount} trees!`);
      loadTrees(); // Refresh the tree list
  ...
  const handleStartPuzzle = (game: any, positionIndex: number) => {
    setCurrentGame(game);
  ...
  return (
    <>
      ...
      {/* Puzzle Interface Modal */}
      {showPuzzleInterface && currentGame && (
        <PuzzleInterface
          game={currentGame}
